...
p1.grids <- list(
  rocks = draw_grid(nodes = n.init$nodes,
                    rocks = r.init$grid,
                    time = round(r.init$time/1e6, 2),
                    p.type = 'rocks',
                    leg.pos = 'bottom') +
  ...
  temperature = draw_grid(nodes = n.init$nodes,
                          rocks = r.init$grid,
                          time = round(r.init$time/1e6, 2),
                          p.type = 'temperature',
                          v.pal = 'magma') +
  ...
  viscosity = draw_grid(nodes = n.init$nodes,
                        rocks = r.init$grid,
                        time = round(r.init$time/1e6, 2),
                        p.type = 'viscosity',
                        v.pal = 'viridis') +
...
suppressWarnings(ggsave(filename = 'figs/fig2.png', device = 'png', type = 'cairo', width = 8, height = 5, bg = 'transparent'))
...
p3.grids <- list(
  hf = draw_grid(hf = mods %>% filter(model == 'cdf78' && tstep == 250) %>% unnest(hf),
                 time = round(r$time1/e6, 2),
                 p.type = 'hf'),
  rocks = draw_grid(nodes = n$nodes,
                    rocks = r$grid,
                    time = round(r$time/1e6, 2),
                    p.type = 'rocks',
                    leg.pos = 'bottom'),
  rocks.zoom = draw_grid(nodes = n$nodes,
                         rocks = r$grid,
                         time = round(r$time/1e6, 2),
                         p.type = 'rocks',
                         leg.pos = 'bottom',
                         box = c(-18, 180, 900, 1600)),
  temperature = draw_grid(nodes = n$nodes,
                          rocks = r$grid,
                          time = round(r$time/1e6, 2),
                          p.type = 'temperature',
                          v.pal = 'magma',
                          box = c(-18, 180, 900, 1600)),
  viscosity = draw_grid(nodes = n$nodes,
                        rocks = r$grid,
                        time = round(r$time/1e6, 2),
                        p.type = 'viscosity',
                        v.pal = 'viridis',
                        box = c(-18, 180, 900, 1600)),
  strain = draw_grid(nodes = n$nodes,
                     rocks = r$grid,
                     time = round(r$time/1e6, 2),
                     p.type = 'strain',
                     v.pal = 'viridis',
                     box = c(-18, 180, 900, 1600)),
  shear = draw_grid(nodes = n$nodes,
                    rocks = r$grid,
                    time = round(r$time/1e6, 2),
                    p.type = 'shear',
                    v.pal = 'viridis',
                    box = c(-18, 180, 900, 1600)),
  stream = draw_grid(nodes = n$nodes,
                     rocks = r$grid,
                     time = round(r$time/1e6, 2),
                     p.type = 'stream',
                     v.pal = 'viridis',
...
p3 <- 
  ...
  (p3.grids$shear + (p3.grids$stream + theme(panel.grid = element_blank(), panel.border = element_blank(), pan))) +
...
suppressWarnings(ggsave(filename = 'figs/fig5.png', plot = p5, device = 'png', type = 'cairo', width = 10, height = 5, bg = 'transparent'))
...
z <- mods$zc
grid %>% 
  mutate(z = predict(lm(z ~ x + y), newdata = grid))
p <-
  p.grid %>% ggplot(aes(y = get(x), x = get(z), z = get(y))) + geom_raster(aes(fill = get(y)),
                                                                           alpha = 0.5, show.legend = F) + geom_contour(
                                                                             aes(color = ..level..),
                                                                             size = 0.3,
                                                                             binwidth = 10,
                                                                             show.legend = F
  ...
  theme_classic()
p1 <-
  direct.label(p, list(fontface = "plain", cex = 0.6, "last.bumpup"))