...
cairo_pdf(width = 5.5, height = 4.25, file = 'FigS1d.pdf')

# Draw Plot
ggplot() +
geom_point(data = subset(antstab, antstab$model == 'cda120'),
           aes(x = time, y = depth, color = model),
           size = 1,
           shape = 15) +
  geom_smooth(
    data = subset(antstab, antstab$model == 'cda120'),
    aes(x = time, y = depth, color = model),
    se = FALSE,
    size = 0.5
  ) +
  geom_point(data = subset(antstab, antstab$model == 'cdf120'),
             aes(x = time, y = depth, color = model),
             size = 1,
             shape = 16) +
  geom_smooth(
    data = subset(antstab, antstab$model == 'cdf120'),
    aes(x = time, y = depth, color = model),
    se = FALSE,
    size = 0.5
  ) +
  geom_point(data = subset(antstab, antstab$model == 'cdp120'),
             aes(x = time, y = depth, color = model),
             size = 1,
             shape = 17) +
  geom_smooth(
    data = subset(antstab, antstab$model == 'cdp120'),
    aes(x = time, y = depth, color = model),
    se = FALSE,
    size = 0.5
  ) +
