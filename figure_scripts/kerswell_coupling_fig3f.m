...
%     set(gcf,'Position',[1 1 1280 720]);
    p = pcolor(gx/1000,gy/1000,log10(eii));
    shading interp;
    caxis([-18 -12]);
    set(p, 'facealpha', bckgrnd_alpha);
    hold on
    [c,h]=contour(gx/1000,gy/1000,(tk-273.15)/300+17,(100:200:2500)/300+17,'LineColor',[1 1 1], 'LineWidth',1);
    if(rect == 1)
    % Rectangular region
    ...
    set(stream1, 'color', [0 0 0 alpha1], 'LineWidth', 0.1);
    ...
    set(stream_circ, 'color', [0 0 0 alphacirc], 'LineWidth', 0.25);
    end
    axis ij image;
    axis([900 1600 -20 180]);
    colorbar;
    ...
    set(gcf,'color','w');
    title(['Log Strain Rate s^{-1} | cdf78 | ' num2str(timesum/1e6, '%.2f'),' Ma']);
    export_fig('Fig3f_cdf78','-png','-r720');
end