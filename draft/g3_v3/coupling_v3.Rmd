...
```{r rock.type}
tibble(
  type = c(1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12, 14),
  name = c('Air', 'Water', 'Sediments', 'Sediments', 'Felsic Crust', 'Felsic Crust', 'Basalt', 'Gabbro', 'Dry Mantle', 'Dry Mantle', 'Hydrated Mantle', 'Serpentinized Mantle')
) %>%
...
map_df(list(bvm.phi.lin,
  ...
  kable_styling(font_size = 8) %>%
  kable_classic() %>%
  footnote(number = c('$z_c=\\\\phi$,', '$z_c=z_{1100}$', '$z_c=z_{1100}^2$', '$z_c=z_{1100}+z_{1100}^2$', '$z_c=z_{1100}+\\\\phi$', '$z_c=z_{1100}^2+\\\\phi$', '$z_c=z_{1100}+z_{1100}^2+\\\\phi$'),
           threeparttable = T,
...
## (De-)hydration model {#sec:dhydration}
